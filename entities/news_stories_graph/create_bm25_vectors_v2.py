r"""Create v2 of the news_stories_graph bm25_vectors entity cache.

Identical to v1 in algorithm, normalization, corpus order, and chunk size.
The only additions are two new persisted artifacts:

    encoder_v2.pkl     - pickled ``pinecone_text.sparse.BM25Encoder`` (fitted)
    idx_map_v2.json    - JSON mapping str(hash_index) -> compact_col_index

These enable :meth:`StoryGraphAPI.bm25_encode_text` to encode arbitrary text
into the same column space as the stored matrix without refitting the encoder.

v2 must produce a matrix bit-equivalent to v1:

    np.allclose(v1.matrix.data, v2.matrix.data)  # True
    (v1.matrix.indices == v2.matrix.indices).all()  # True
...
``[0, K)`` range, L2-normalizes rows, and persists five artifacts via
``Bm25VectorsCache.put``:

    bm25_v2.npz        - L2-normalized (N x K) CSR matrix
    params_v2.json     - encoder hyperparameters + corpus stats
    story_ids_v2.json  - row-ordered story_ids aligned to the matrix
    encoder_v2.pkl     - pickled fitted BM25Encoder
    idx_map_v2.json    - JSON { str(hash_idx): compact_col } for query encoding
...
Usage::
    ...
    python -m entities.news_stories_graph.create_bm25_vectors_v2
"""
...
def _encode_to_coo(
    encoder: BM25Encoder,
    texts: list[str],
) -> tuple[coo_matrix, int, dict[int, int]]:
    """Encode ``texts`` in CHUNK_SIZE blocks, building COO incrementally.

    Returns (coo_matrix, n_cols, idx_map) where the matrix uses a compacted
    hashed vocabulary (columns ``[0, n_cols)``).  ``idx_map`` maps each raw
    encoder hash index to its compact column index and is needed for
    query-time encoding.
    """
    n = len(texts)
    all_rows: list[int] = []
...
def create() -> None:
    """Build news_stories_graph bm25_vectors v2 end-to-end."""
    ...
    raw, n_cols, idx_map = _encode_to_coo(encoder, texts)
    ...
    # Persist encoder via pickle (BM25Encoder does not have a built-in save).
    logger.info("Pickling fitted BM25Encoder (%d docs idf table)", n)
    t_pickle = time.monotonic()
    encoder_bytes = pickle.dumps(encoder, protocol=pickle.HIGHEST_PROTOCOL)
    logger.info(
        "Encoder pickle: %.1f MB in %.1fs",
        len(encoder_bytes) / 1_048_576,
        time.monotonic() - t_pickle,
    )
    ...
    params: dict[str, object] = {
        ...
        "encoder_pkl_size_bytes": len(encoder_bytes),
        "idx_map_len": len(idx_map),
    }

    logger.info("Persisting to Bm25VectorsCache (local + S3)")
    bm25_vectors_cache.put(
        matrix,
        params,
        story_ids,
        version=VERSION,
    ...
    logger.info("=" * 60)
    logger.info("news_stories_graph bm25_vectors v2 build complete")
    ...
    logger.info("  idx_map:  %d entries", len(idx_map))
