"""run_01_gpu: 5% probe variant calling harness_gpu (MLX) instead of harness.

Mirror of run_01.py but routes the two bootstrap functions through Apple Silicon
Metal via MLX. Primary metric expected to match CPU baseline (0.4787) to 1e-3.
Wall expected ~150-300s based on smoke extrapolation (vs ~1856s pre-Tier-1 CPU).
"""
